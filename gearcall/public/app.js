...
let signupSpec = 'car';
let resetTokenFromUrl = null; // set on boot if the page was opened from a reset-password email link
...
function setAuthTab(tab) {
  // tab is one of: 'login', 'signup', 'forgot', 'reset'
  document.getElementById('tab-login').classList.toggle('active', tab === 'login');
  document.getElementById('tab-signup').classList.toggle('active', tab === 'signup');
  // The tab bar only makes sense for login/signup — hide it for the forgot/reset screens.
  document.getElementById('auth-tabs').style.display = (tab === 'forgot' || tab === 'reset') ? 'none' : 'flex';
...
function hideAuthNotice() {
  document.getElementById('auth-notice').classList.remove('show');
}
...
async function doForgotPassword() {
  ...
  if (!email) return showAuthError('Enter your account email.');
  const btn = event && event.target;
  if (btn) btn.disabled = true;
  try {
    const data = await api('/api/auth/forgot-password', { method: 'POST', body: { email } });
    showAuthNotice(data.message || 'If that email is registered, a reset link has been sent.');
  } catch (e) {
    showAuthError(e.message);
  } finally {
...
async function doResetPassword() {
  ...
  if (!resetTokenFromUrl) return showAuthError('This reset link is missing its token. Please request a new one.');

  try {
    await api('/api/auth/reset-password', { method: 'POST', body: { token: resetTokenFromUrl, password } });
    resetTokenFromUrl = null;
    // Clean the ?reset=... param out of the URL bar.
...
(async function boot() {
  ...
  // (…/?reset=TOKEN), jump straight to the reset-password screen.
